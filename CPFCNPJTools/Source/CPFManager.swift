...
public class CPFManager {
    ...
    /// Generates a random valid CPF, without formatting.
    ///
    /// - Returns: A valid CPF, but unformatted.
    ///
    ...
    /// ```swift
    /// let cpfManager = CPFManager()
    /// let cpf = cpfManager.generate()
    /// print(cpf)  // Randomly generated CPF
    /// ```
    ...
    /// Formats a CPF in the traditional format `XXX.XXX.XXX-XX`.
    ///
    /// - Parameter cpf: The CPF to be formatted. The CPF must have exactly 11 numeric digits.
    /// - Returns: The formatted CPF as a string or `nil` if the CPF does not have 11 digits.
    ///
    /// **Example:**
    /// ```swift
    /// let cpfManager = CPFManager()
    /// let formattedCpf = cpfManager.format("11144477735")
    /// print(formattedCpf)  // "111.444.777-35"
    /// ```
